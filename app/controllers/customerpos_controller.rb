class CustomerposController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @customerpo.save
        format.html { redirect_to root_path, notice: 'Customerpo was successfully created.' }
  ...
  def destroy
    @customerpo.destroy
    respond_to do |format|
      format.html { redirect_to root_path, notice: 'Customerpo was successfully destroyed.' }
  ...
  private
    ...
    def customerpo_params
      params.require(:customerpo).permit(:number, :date, :amount, :customerquote_id, :scope_id)
    end
end
